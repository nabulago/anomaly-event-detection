...
'''
	...
	The motion features are in color and grayscale format
...
def dispOpticalFlow(Image,  Flow,  Divisor,  name):
	...
	return []
winW,  winH = 15, 15
parser = argparse.ArgumentParser(description='Optical Flow segmentation')
parser.add_argument('-dp', '--dpath', help='Dataset Training folder path')
parser.add_argument('-ws', '--winsize', help='Size of the window')
args = parser.parse_args()
dpath = args['dpath']

#print args
# Please add path to the training folder
# Path is hardcoded not taken for the terminal arguments
# put this file in the test train folder and it will 
...
#	index = 0
	...
	for index, file in enumerate(files):
		...
		if index<=198:
			...
			if os.path.exists(os.path.join(str(datasetPath), str(f), str(files[index]))):
				prevFrame = cv2.imread(os.path.join(str(datasetPath), str(f), str(files[index])))
			else:
				break
			#print index+1
			if os.path.exists(os.path.join(str(datasetPath), str(f), str(files[index+1]))):
				nextFrame = cv2.imread(os.path.join(str(datasetPath), str(f), str(files[index+1])))
			else:
				break
			...
			tmppath = os.path.join(str(datasetPath), str(f))
			...
			dispOpticalFlow(prevFrame, flow, 10, 'Optical flow')
			#print "Optical flow"
			winNo = 0
			rgbImg = cv2.resize(rgbImg, (240, 165), 1)
			motionGray = cv2.resize(motionGray, (240, 165), 1)
			for (x,  y,  window) in sliding_window(rgbImg,  stepSize=15,  windowSize=(15,  15)):
				...
				# print "windows pre"
				# print "Windows pre shape : " +str(window.shape)
			...
			winNo1 = 0
			motionGray = cv2.resize(motionGray, (240, 165), 1)
			for (x1,  y1,  window1) in sliding_window(motionGray,  stepSize=15,  windowSize=(15,  15)):
...
os._exit(0)
# print 'end of script'
	# if file != '.DS_Store' or '._.DS_Store':
