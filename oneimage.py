			...
			for file in os.listdir(imageDir+"/"+str(folder)):
				...
				if image is not None:
					...
					for sz in winSz:
						...
						if sz == 15:
							resized = cv2.resize(win,(240,165), interpolation = cv2.INTER_CUBIC)
							# resized = resizeToMainWindowSize(image,240, 165)
						elif sz == 18:
							resized = cv2.resize(win,(252,162), interpolation = cv2.INTER_CUBIC)
							# resized = resizeToMainWindowSize(image,252, 162)
						elif sz == 20:
							resized = cv2.resize(win,(240,160), interpolation = cv2.INTER_CUBIC)
							# resized = resizeToMainWindowSize(image,240,160)

						for (x, y, window) in sliding_window(resized, stepSize= tz, windowSize=(tz, tz)):
						...
						# if the window does not meet our desired window size, ignore it
							...
							if sz==15:
								...
								fObject.writelines(folder+ "/" + str(sz)+"/" + str(imgCount)+"_"+str(sz)+"_"+file.split('.')[0]+".jpeg"+"\n")
								f_main.writelines(folder+ "/" + str(sz)+"/" + str(imgCount)+"_"+str(sz)+"_"+file.split('.')[0]+".jpeg"+"\n")
							...
							if sz == 18:
								#print "In loop of 18"
								# nm1 = dataDest+"/"+ folder+ "/" + str(sz)+"/" + str(imgCount)+"_"+str(sz)+"_"+file.split('.')[0]+".jpeg" 
								nm2 = dataDest+"/"+ folder+ "/" + "rs"+str(sz)+"/" + str(imgCount)+"_"+str(sz)+"_15_"+file.split('.')[0]+".jpeg" 
								fObject.writelines(folder+ "/" + str(sz)+"/" + str(imgCount)+"_"+str(sz)+"_"+file.split('.')[0]+".jpeg"+"\n")
								f_main.writelines(folder+ "/" + "rs"+str(sz)+"/" + str(imgCount)+"_"+str(sz)+"_15_"+file.split('.')[0]+".jpeg"+"\n")
								# print nm1
